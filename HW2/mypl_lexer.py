...
#
# all tokens and some exceptions
#     return the column of the beginning of the lexeme
class Lexer(object):
    # init method
    ...
    # return the character and advances the position
    def __read(self):
    ...
    def next_token(self):
        ...
        # if digit, determine if float or int, then return
            ...
            # read until it's another important character in MyPL or whitespace
            ...
            while self.__peek() not in ";=+-*/%<>()" and not (
                ...
                if not self.__peek().isdigit() and self.__peek() != ".":
                    raise error.MyPLError('unexpected symbol "' + (
                        self.__peek()) + '"', self.line, self.column)
                
                # if decimal point, ensure this is the first one
                # and that the first half of the number is a valid intval
                elif self.__peek() == "." and not isFloat:
                    if len(curr_lexeme) == 1 or (curr_lexeme[0] != "0" and (
                        len(curr_lexeme)) > 1):
                        isFloat = True
                    else:
                        raise error.MyPLError("float starts with invalid int", (
        ...
        # if alpha, determine if ID or keyword and return accordingly
            ...
            while self.__peek().isalpha() or self.__peek().isdigit() or (
                self.__peek() == "_"):
        ...
        # if any other recognized character in MyPL
            ...
            if self.__peek() == "=":
                ...
                return token.Token(token.NOT_EQUAL, "!=", self.line, (
                 ...
                 # -1 to column to go back one since we read it already
                raise error.MyPLError('unexpected symbol "!"', self.line, 
                    (self.column - 1))
        elif peekValue == "(":
            self.__read()
